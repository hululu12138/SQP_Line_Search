function [x,mu,lam,val,k,info]=svmsqp(x0,mu0,lam0,xTr,yTr)
...
[hk,gk]=cons(x,yTr); dfk=df1(x,xTr,yTr);
[Ae,Ai]=dcons(x,yTr); Ak=[Ae; Ai];
k=0;
while(k<maxk)
    ...
    im=0; %Armijo搜索 
    while(im<=20)
        if(phi1(x+rho^im*dk,sigma,xTr,yTr)-phi1(x,sigma,xTr,yTr)<eta*rho^im*dphi1(x,sigma,dk,xTr,yTr))
            mk=im;
            break; 
        end
        im=im+1;
    ...
    sk=alpha*dk; %更新矩阵Bk 
    yk=dlax(x1,mu,lam,xTr,yTr)-dlax(x,mu,lam,xTr,yTr); 
...
%dd=norm(dk)
%%%%%%%%%%%%%%%%%%%%%% l1精确价值函数 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function p=phi1(x,sigma,xTr,yTr)
f=f1(x,xTr,yTr); [h,g]=cons(x,yTr); gn=max(-g,0); 
...
%%%%%%%%%%%%%%%%%%%%%%%%%% 价值函数的方向导数%%%%%%%%%%%%%%%%%%%%%%%%%%
function dp=dphi1(x,sigma,d,xTr,yTr)
df=df1(x,xTr,yTr); [h,g]=cons(x,yTr); gn=max(-g,0); 
l0=length(h); m0=length(g);
...
%%%%%%%%% 拉格朗日函数的梯度 %%%%%%%%%%%%% 
function dl=dlax(x,mu,lam,xTr,yTr)
df=df1(x,xTr,yTr); %调用目标函数梯度文件 
[Ae,Ai]=dcons(x,yTr); %调用约束函数Jacobi矩阵文件
